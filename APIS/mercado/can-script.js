   ...
   // });
    ...
    request2.onload = function() {
      if (request2.status !== 200 || request2.readyState !== 4) {
        console.log('Network request for "' + product.name + '" image failed with response ' + request2.status + ': ' + request2.statusText);
      } else {
        var objectURL = URL.createObjectURL(request2.response); //creando una url temporal que almacena la imagen en el servidor de la pagina
        showProduct(objectURL, product);
      }
    }
  }
